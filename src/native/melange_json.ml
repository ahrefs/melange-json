...
(** Standard payload-preserving record for [@json.open_enum] catch-all
    constructors on polymorphic and classic variants. The PPX generates
    record literals of this shape; users can reference this type directly
    instead of defining their own. Wire shapes:
...
(* Companion schema for ppx_deriving_jsonschema callers. Structurally
   ...
   of any" — the two wire shapes [open_enum] can take. *)
let open_enum_jsonschema =
  `Assoc
    [
      ( "oneOf",
        `List
