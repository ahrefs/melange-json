...
module Of_json = struct
  ...
  (* Build a payload-preserving open_enum value from the already-extracted
  ...
  let build_open_enum_record ~loc =
    [%expr
      let tag_s = tag in
      let payload =
    ...
    | Vcs_tuple (n, t) when vcs_attr_json_open_enum t.tpl_ctx ->
        ...
        (match t.tpl_types with
         | [ { ptyp_desc = Ptyp_constr ({ txt = Lident "string"; _ }, []); _ } ] ->
             (* Tag-only form: capture the bare string tag. Array forms
                with unknown tag also reach here; the payload is dropped. *)
             make (Some [%expr tag])
         | [ _ ] -> make (Some (build_open_enum_record ~loc))
         | _ ->
    ...
    | Vcs_record (_n, t) when vcs_attr_json_open_enum t.rcd_ctx ->
        ...
        (match t.rcd_fields with
         | [ { pld_name = { txt = "tag"; _ }; _ };
             ...
             make (Some (build_open_enum_record ~loc))
         | _ ->
             Location.raise_errorf ~loc
